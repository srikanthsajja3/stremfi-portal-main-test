...
const OperatorList = () => {
  ...
  const fetchOperators = async () => {
    try {
      const { data } = await apiClient.get('/operator_api.php?filterRole=operator');
  ...
  const fetchOperatorsSearch = async (name = '') => {
    setLoading(true);
    try {
      const url = `/searchOperatorsByName.php?name=${encodeURIComponent(name)}&role=operator`;
      ...
      if (response.data && response.data.status === 'success') {
        ...
        setOperators([]);
        // console.warn('No operators found or unexpected response:', response.data);
      }
    } catch (error) {
  ...
  const handleAddBalance = async () => {
    ...
    if (selectedOperator.created_by !== user.operatorId) {
      if (!window.confirm("Are you sure, you want Add Balance to admin operator?")) return;
    }
  ...
  const openEditOperatorForm = (operator) => {
    setEditForm({
      ...
      // bcrypt hash (or, for an operator who hasn't logged in since the
      ...
      // account's login. Left blank; the backend already keeps the
      // existing password when this field is empty.
      password: '',
  ...
  const handleEditChange = (e) => {
    ...
    setEditForm((prev) => ({
      ...prev,
      [name]: value,
      // Changing partner invalidates whichever branch was picked under
      // the old partner.
  ...
  return (
    <>
      ...
      <div className="operator-header">
        ...
        <input
          type="text"
          placeholder="Search Username, Network, Name, Phone"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="search-input"
        />
        <button className="add-operator-btn" onClick={() => navigate('/add-operator')}>
          + Add Operator
        </button>
      </div>
      ...
      {error ? (
        ...
        loading ? (<>
                ...
                <div className="modal-content operator-profile-modal">
                  ...
                  <div className="operator-profile-content">
                    ...
                    <div className="oper-buttons">
                      <button className='oper-btn oper-btn-edit' onClick={() => openEditOperatorForm(selectedOperator)}>
                        Edit Profile
                      </button>
                      <button className='oper-btn oper-btn-balance' onClick={() => openAddBalanceForm(selectedOperator)}>
                        Add Balance
                      </button>
                      <button onClick={() => tempLoginOperator(selectedOperator.id)} className="oper-btn oper-btn-login">
                        Login as Operator
                      </button>
                    </div>
                  </div>
              ...
              <div className="modal-overlay">
                <div className="modal-content">
                  ...
                  <div className="modal-buttons">
                    <button className="btn" onClick={handleAddBalance}>Submit</button>
                    <button className="btn cancel-btn" onClick={() => { setShowBalanceForm(false); setBalancePassword(''); }}>Cancel</button>
                  </div>
                </div>
              </div>
            )}
                  ...
                  {isSuperAdmin && (
                    <>
                      ...
                      {isSuperAdmin ? (
                        ...
                        <input
                          value={selectedOperator.created_by_name}
                          readOnly
                        />
                      )}
          ...
          <input name="operCode" placeholder="OTTPlay Operator Code" value={editForm.operCode} onChange={handleEditChange} /> */}
                  ...
                  <div className="modal-buttons">
                    <button className="btn" onClick={submitEditForm}>Update</button>
